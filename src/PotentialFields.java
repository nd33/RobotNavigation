...
public class PotentialFields
{
	...
	private final int robotInitialHeadingId;
	...
	public PotentialFields() {
		...
		gui.addLabel(2, 8, "Robot Initial Heading:");
		robotInitialHeadingId = gui.addTextField(2, 9, null);
		...
		//More options
		gui.addLabel(6, 0, "Planner: ");
		enArcId = gui.addButton(6, 1, "Arcs", this, "enableArcs");
		disArcId = gui.addButton(6, 2, "Steepest Descend", this, "disableArcs");
	...
	public void clearButtonAction() {
		...
		gui.setTextFieldContent(robotInitialHeadingId, "");
	...
	public void buttonAction() throws InterruptedException {
		...
		String robotInitialHeading = gui.getTextFieldContent(robotInitialHeadingId);
		...
		double robotHeading;
		if(robotInitialHeading.equals(""))
			robotHeading = rand.nextInt(360);
		else
			robotHeading = Double.parseDouble(robotInitialHeading);
		//if heading is is not between 0 and 360 set a random heading
		if(robotHeading < 0 || robotHeading > 360)
			robotHeading = rand.nextInt(360);
		robotHeading = robotHeading * Math.PI / 180; // convert to radians

		goLittleRobot(new IntPoint(startX, startY), new IntPoint(goalX, goalY), radius, 
				      robotRadius, robotSensorRange, robotSensorDensity, robotSpeed, image, robotHeading);
	}
	...
	public void goLittleRobot(IntPoint start, IntPoint goal, int goalRad, 
			                  ...
			                  int robotSpeed, String image, double robotInitialHeading) throws InterruptedException
	{
		...
		PotentialFieldsRobot rob = new PotentialFieldsRobot(image, start, goal, robotRadius, robotSensorRange, 
							  robotSensorDensity, goalRad, obstacles, robotInitialHeading);
		...
		while (!rob.inGoal()) {
			...
			boolean move = rob.move(arcs); //Move 1 step
        ...
        System.out.println(PotentialFieldsRobot.movesMade);
        PotentialFieldsRobot.movesMade = 0;
	...
	private void drawRobot(PotentialFieldsRobot rob)  {
		...
		LinkedList<IntPoint> pts = rob.getArcPoints();
		RenderablePolyline lines = new RenderablePolyline();
		for (IntPoint p : pts) {
			lines.addPoint(p.x, p.y);
		}
		lines.setProperties(Color.CYAN, 3f);
		gui.draw(lines);
	}
